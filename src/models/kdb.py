...
class KDB(GeneralRecommender):
    def __init__(self, config, dataset):
        super(KDB, self).__init__(config, dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        # ==========================================
        # 新增：自适应教师权重计算 (Adaptive Teacher Weighting)
        # 基于每个样本的预测差异计算教师置信度
        # ==========================================
        with torch.no_grad(): # 教师权重不需要梯度
            stacked_t_diff = torch.stack([t1_diff, t2_diff], dim=1) # 形状: [batch_size, 2]
            # 使用 temp 进行平滑控制。temp 越大权重越平均，temp 越小越倾向于置信度高的教师
            teacher_weights = F.softmax(stacked_t_diff / self.temp, dim=1)
            w1, w2 = teacher_weights[:, 0], teacher_weights[:, 1] # 形状: 分别为 [batch_size]

        # 3. 蒸馏损失 Hinge Loss (样本级计算并应用权重)
        ...
        # 4. 蒸馏损失 Cross Entropy Loss (样本级计算并应用权重)
