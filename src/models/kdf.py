...
class KDF(GeneralRecommender):
    def __init__(self, config, dataset):
        # 初始化学生模型 (继承 ALLB，即学生自身使用 ALLB 架构)
        super(KDF, self).__init__(config, dataset)
        # ==========================================
        # 1. 蒸馏相关的超参数 (新增动态权重)
        # ==========================================
    ...
    def calculate_loss(self, interaction, edge_weight=None):
        # 【修改函数】支持传入扰动 edge_weight，以及自适应权重调控蒸馏机制
        user = interaction[0]
        ...
        # ==========================================
        # 4. 【核心修改】自适应权重调控 (Adaptive Weight Control)
        # ==========================================
        # 计算学生和教师的样本级 BPR Loss
        s_loss_sample = -F.logsigmoid(s_diff)
        t1_loss_sample = -F.logsigmoid(t1_diff)
        t2_loss_sample = -F.logsigmoid(t2_diff)

        # 4.1 教师间相对分配：基于“学生-教师”表现差距 (Student-Teacher Gap)
        # Gap越大，说明该教师在这个样本上比学生领先得越多，越需要向其学习
        adaptive_tau = getattr(self, 'adaptive_tau', 1.0)  # 控制锐度的温度超参，可后续在配置中添加
        gap1 = (s_loss_sample - t1_loss_sample) / adaptive_tau
        gap2 = (s_loss_sample - t2_loss_sample) / adaptive_tau

        # 利用 Softmax 进行教师间权重的自适应分配
        teacher_weights = F.softmax(torch.stack([gap1, gap2], dim=1), dim=1)
        w1 = teacher_weights[:, 0]
        w2 = teacher_weights[:, 1]

        # 4.2 整体自适应蒸馏强度：基于学生的困惑度 (Student Status)
        # 学生自身的 loss 越大，说明越需要老师的指导；loss 极小说明已经掌握，应降低蒸馏权重
        # 使用 tanh 进行平滑映射，并加 detach 阻止梯度流向学生的主干预测逻辑
        adaptive_scale = torch.tanh(s_loss_sample.detach())
        ...
        # 叠加自适应总体强度 adaptive_scale 与 相对权重 w1/w2
        hinge_loss = self.alpha_hinge * torch.mean(
            adaptive_scale * (w1 * hinge_loss_1_sample + w2 * hinge_loss_2_sample)
        )

        # ==========================================
        # 6. 自适应加权的 Cross Entropy 蒸馏损失
        # ==========================================
        ...
        # 同样叠加双重自适应控制
        ce_loss = self.beta_ce * torch.mean(
            adaptive_scale * (w1 * ce_loss_1_sample + w2 * ce_loss_2_sample)
        )
