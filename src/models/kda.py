...
class KDA(GeneralRecommender):
    def __init__(self, config, dataset):
        super(KDA, self).__init__(config, dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        # 3. 教师模型前向传播 (使用克隆对象)
        with torch.no_grad():
            # 核心修复点：使用 interaction.clone() 确保教师的操作不影响原始数据
            # 如果你的框架不支持 .clone()，请使用 copy.copy(interaction)
        ...
        # 3. 蒸馏损失 (引入 alpha 控制)
        hinge_loss_1 = torch.mean(torch.clamp(t1_diff - s_diff, min=0.0))
        hinge_loss_2 = torch.mean(torch.clamp(t2_diff - s_diff, min=0.0))
        hinge_loss = self.alpha_hinge * (hinge_loss_1 + hinge_loss_2)

        # 4. 蒸馏损失 (Cross Entropy Loss，引入 beta 控制)
        t1_bar = torch.sigmoid(t1_diff / self.temp)
        t2_bar = torch.sigmoid(t2_diff / self.temp)
        s_bar = torch.sigmoid(s_diff / self.temp)

        eps = 1e-8
        ce_loss_1 = -torch.mean(t1_bar * torch.log(s_bar + eps) + (1 - t1_bar) * torch.log(1 - s_bar + eps))
        ce_loss_2 = -torch.mean(t2_bar * torch.log(s_bar + eps) + (1 - t2_bar) * torch.log(1 - s_bar + eps))
        ce_loss = self.beta_ce * (ce_loss_1 + ce_loss_2)
