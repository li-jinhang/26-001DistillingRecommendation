...
class MADG(GeneralRecommender):
    def __init__(self, config, dataset):
        super(MADG, self).__init__(config, dataset)
    ...
    def calculate_alignment_loss(self, users):
        ...
        # 4. 执行均值与方差的粗粒度对齐
        # (1) 多模态与 ID 对齐 (主对齐)
        loss_m_id = self.mean_var_align_loss(z_m, z_id)

        # (2) 单模态与 ID 对齐 (增加视觉、文本与用户ID之间的方差对齐)
        loss_v_id = self.mean_var_align_loss(z_v, z_id)
        loss_t_id = self.mean_var_align_loss(z_t, z_id)

        # (3) 模态间对齐 (消除视觉与文本之间的 Modality Gap)
        loss_v_t = self.mean_var_align_loss(z_v, z_t)
