...
class MADH(GeneralRecommender):
    def __init__(self, config, dataset):
        super(MADH, self).__init__(config, dataset)
    ...
    def mean_var_align_loss(self, view1, view2):
        """
        通过均值和方差进行粗粒度分布对齐
        ...
        # 1. 计算均值 (Mean) 对齐
        mean1 = view1.mean(dim=0)
        mean2 = view2.mean(dim=0)
        loss_mean = F.mse_loss(mean1, mean2)

        # 2. 计算方差 (Variance) 对齐
        # 分别计算两个视图在每个特征维度上的方差 (相当于原协方差矩阵的对角线)
        var1 = view1.var(dim=0)
        var2 = view2.var(dim=0)
    ...
    def calculate_alignment_loss(self, users):
        ...
        # 获得最大池化后的视觉嵌入矩阵 (pool_v) 和 文本嵌入矩阵 (pool_t)
        ...
        # 3. 提取初始的用户嵌入矩阵
        # (在GCN初始化中，preference 即为纯 ID 的初始用户嵌入)
        u_init = self.id_gcn.preference[users]

        # 4. 仿照 MENTOR，计算各嵌入矩阵的全局标量方差(var)与均值(mean)
        v_var = torch.var(pool_v)
        v_mean = torch.mean(pool_v)

        t_var = torch.var(pool_t)
        t_mean = torch.mean(pool_t)

        u_var = torch.var(u_init)
        u_mean = torch.mean(u_init)

        # 5. 计算 MENTOR 风格的绝对值差异对齐损失 (全局宏观对齐)
        # (1) 视觉模态与初始用户对齐
        loss_v_u = torch.abs(u_var - v_var) + torch.abs(u_mean - v_mean)
        # (2) 文本模态与初始用户对齐
        loss_t_u = torch.abs(u_var - t_var) + torch.abs(u_mean - t_mean)
        # (3) 视觉与文本模态间互相对齐
        loss_v_t = torch.abs(v_var - t_var) + torch.abs(v_mean - t_mean)

        # 汇总所有对齐损失
        align_loss = loss_v_u + loss_t_u + loss_v_t
