...
class MADF(GeneralRecommender):
    def __init__(self, config, dataset):
        super(MADF, self).__init__(config, dataset)
        ...
        # ========= 修改：保存图的边索引，用于后续的 Max Pooling =========
        ui_rows = train_interactions.row
        ui_cols = train_interactions.col

        # 保存到 device 供 calculate_alignment_loss 使用
        self.ui_rows_tensor = torch.LongTensor(ui_rows).to(self.device)
        self.ui_cols_tensor = torch.LongTensor(ui_cols).to(self.device)
    ...
    def info_nce_loss(self, view1, view2, temp=0.2):
        """
        view1: 模态1的向量 (batch_size, dim)
        view2: 模态2的向量 (batch_size, dim)
        temp: 温度参数
        """
        ...
        # 计算两组向量之间的余弦相似度矩阵
        pos_score = torch.sum(view1 * view2, dim=1)  # (batch_size,)
        ttl_score = torch.matmul(view1, view2.transpose(0, 1))  # (batch_size, batch_size)

        # InfoNCE 公式计算
        pos_score = torch.exp(pos_score / temp)
        ttl_score = torch.sum(torch.exp(ttl_score / temp), dim=1)
    ...
    def calculate_alignment_loss(self, users):
        ...
        # 2. Max Pooling 聚合用户感兴趣的物品特征
        pool_id_all, _ = scatter_max(id_item[self.ui_cols_tensor], self.ui_rows_tensor, dim=0, dim_size=self.num_user)
        ...
        pool_m = torch.cat((pool_v, pool_t), dim=1)  # 128维

        # 3. 投影到统一的 64 维空间 (这一步至关重要，它对齐了维度)
        # 既然改用了 InfoNCE，这些 Linear 层就充当了对比学习中的 Projection Head
        z_id = self.mu_id(pool_id)  # 64维
        z_v = self.mu_v(pool_v)  # 64维
        z_t = self.mu_t(pool_t)  # 64维
        z_m = self.mu_m(pool_m)  # 64维 (从128映射到64)

        # 4. 执行 InfoNCE 对比对齐
        # (1) 多模态与 ID 对齐 (主对齐)
        loss_m_id = self.info_nce_loss(z_m, z_id, self.temp)

        # (2) 单模态与 ID 对齐
        loss_v_id = self.info_nce_loss(z_v, z_id, self.temp)
        loss_t_id = self.info_nce_loss(z_t, z_id, self.temp)

        # (3) 模态间对齐 (消除视觉与文本之间的 Modality Gap)
        loss_v_t = self.info_nce_loss(z_v, z_t, self.temp)

        # 汇总损失
        align_loss = loss_m_id + 0.5 * (loss_v_id + loss_t_id + loss_v_t)
