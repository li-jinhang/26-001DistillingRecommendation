...
class KDH(GeneralRecommender):
    def __init__(self, config, dataset):
        super(KDH, self).__init__(config, dataset)
        # ==========================================
        # 1. 蒸馏相关的超参数 (保持不变)
        # ==========================================
        ...
        if not t1_weight or not t2_weight:
            raise RuntimeError('KDH 缺少教师模型配置，请在 KDH.yaml 中提供 teacher1_weight 和 teacher2_weight。')
    ...
    def _resolve_teacher_weight_path(self, weight_path):
        if os.path.isabs(weight_path):
            if os.path.exists(weight_path):
                return weight_path
            raise FileNotFoundError(f'KDH 教师权重不存在: {weight_path}')
        ...
        for candidate in candidates:
            if os.path.exists(candidate):
                return candidate
        raise FileNotFoundError(f'KDH 教师权重不存在: {candidates[2]}')
    ...
    def _extract_state_dict(raw_state):
        ...
        if not isinstance(state, dict):
            raise RuntimeError(f'KDH 教师权重格式非法: {type(raw_state)}')
    ...
    def _build_teacher_embedding_cache(self, teacher_name, teacher_weight, config, dataset):
        ...
        if shape_mismatch_keys:
            raise RuntimeError(
                f'KDH 教师 `{teacher_name}` 权重维度不匹配: {shape_mismatch_keys[:5]}'
            )
        if unexpected_keys:
            print(f'WARNING: KDH 教师 `{teacher_name}` 存在未使用权重参数，已自动忽略: {unexpected_keys[:5]}')

        incompatible = teacher_model.load_state_dict(compatible_state, strict=False)
        missing_keys = list(getattr(incompatible, 'missing_keys', []))
        if missing_keys:
            print(f'WARNING: KDH 教师 `{teacher_name}` 存在缺失参数，使用兼容加载: {missing_keys[:5]}')
        ...
        with torch.no_grad():
            ...
            if not hasattr(teacher_model, 'result_embed') or teacher_model.result_embed is None:
                raise RuntimeError(f'KDH 教师 `{teacher_name}` 未生成 result_embed，无法执行蒸馏。')
